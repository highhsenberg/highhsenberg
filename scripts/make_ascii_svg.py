...
CONTRAST = 1.15
BRIGHTNESS = 1.0
GAMMA = 0.58          # <1 brightens mids -> face lands in sparser chars
SHARPEN = False
WHITE_FLOOR = 0.85    # luminance above this is forced to blank (space)
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">highhsenberg@github: ~$ ./portrait.sh</text>')
...
status_prefix = "highhsenberg@github:~$ whoami "
status_name = "VISAL NKR"
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{status_prefix}<tspan fill="{INK}">{status_name}</tspan></text>')
cursor_x = PAD + (len(status_prefix) + len(status_name)) * 5.6
